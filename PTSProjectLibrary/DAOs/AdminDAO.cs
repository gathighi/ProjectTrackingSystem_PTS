...
namespace PTSProjectLibrary.DAOs
{
    internal class AdminDAO : SuperDAO
    {
        public int Authenticate(string username, string password)
        {
            ...
            SqlCommand cmd;
            SqlDataReader dr;
            sql = String.Format("SELECT UserID FROM person WHERE Is_Administrator=1 AND Username='{0}' AND Persons_Password='{1}'", username, password);

            cn = new SqlConnection(Properties.Settings.Default.PTS_DatabaseConnectionString);
        ...
        public List<project> GetListOfProjects(int adminId)// int adminId
        {
            string sql;
            SqlConnection cn;
            SqlCommand cmd;
            ...
            sql = "SELECT * FROM project WHERE AdministartorID= " + adminId; //WHERE AdministratorID = adminId
            cn = new SqlConnection(Properties.Settings.Default.PTS_DatabaseConnectionString);
            cmd = new SqlCommand(sql, cn);
            try
            {
